...
# s3 things
## UPDATE NEXT LINE
S3_BUCKET = 'epj7rf-dp1-spotify'
s3 = boto3.client('s3')

# base URL for accessing the files
## UPDATE NEXT LINE
baseurl = 'http://epj7rf-dp1-spotify.s3-website-us-east-1.amazonaws.com/'
...
def s3_handler(event):
  if _is_json(event.key):
    ...
    # try to insert the song into the databasedb = mysql.connector.connect(user=DBUSER, host=DBHOST, password=DBPASS, database=DB)
